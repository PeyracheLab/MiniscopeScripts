...
results_directory = [data_directory '/' group '/' number '/CellReg'];
% Dimensions for A0634
dims = [136,166];

figures_directory = [data_directory '/' group '/' number '/CellReg/Figures'];
...
while ischar(tline)
    if ~strcmp(tline(1), '#') & ~strcmp(tline(1), ',')            
        ...
        ymd = join(ymd, '');
        ymd = ymd{1};        
        file_names{end+1} = [data_directory '/' group '/' number '/' number '-' ymd(3:end) '/' number '-' ymd(3:end) '_A.csv'];
    end
    tline = fgetl(fid);
end
